...
export const PageA = () => {
  ...
  const [currentResizeId, setCurrentResizeId] = React.useState<string | number>('file');
  ...
  const updateColumnSizing = (columnId: string | number, width: number) => {
    setColumnSizingOptions((state) => ({
      ...state,
      [columnId]: {
        ...state[columnId],
...
const ResizeDialog = (props: Omit<DialogProps, 'children'> & { columnId: string | number; columnWidth?: number; onResize: (width: number) => void }) => {
  const { open, columnId, columnWidth = 100, onOpenChange, onResize } = props;
      ...
      <DialogSurface>
        <DialogBody>
          ...
          <DialogActions>
            ...
            <DialogTrigger disableButtonEnhancement>
              <Button appearance="primary" onClick={() => onResize(newWidth)}>Confirm</Button>
            </DialogTrigger>
          </DialogActions>
        </DialogBody>
      </DialogSurface>
